...
export default async function ReferralsPage() {
  try {
    ...
    return <>
      <PageHeader eyebrow="Growth & payouts" title="Referral earnings" description="Every referred-user purchase earns the referrer Tk 100. Review purchases and transaction IDs before paying bKash withdrawals." />
      ...
      <div className="table-panel referral-withdrawals"><div className="panel-head"><h2>Withdrawal requests</h2></div><div className="withdrawal-cards">{withdrawals.map((item) => <article key={item.id} className={item.status === "pending" ? "withdrawal-card pending" : "withdrawal-card"}>
        ...
        <Link className="evidence-link" href={`/referrals/${item.id}`}>View purchases &amp; transaction IDs</Link>
    ...
    return <><PageHeader eyebrow="Growth & payouts" title="Referral earnings" description="Referral purchases and withdrawals." /><ErrorPanel message={error instanceof Error ? error.message : "Unknown error."} /></>;
  }
}
